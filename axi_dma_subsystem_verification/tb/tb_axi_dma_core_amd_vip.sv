...
module tb_axi_dma_core_amd_vip;
    ...
    // Handshake interposer signals keep the AMD master and DUT views
    // consistent while a directed test holds one AXI-Lite channel.
    logic vip_axil_awvalid;
    logic vip_axil_awready;
    logic vip_axil_wvalid;
    logic vip_axil_wready;
    logic vip_axil_bvalid;
    logic vip_axil_bready;
    logic vip_axil_rvalid;
    logic vip_axil_rready;
    ...
    axil_cpu_vip u_axil_cpu_vip (
        ...
        .m_axi_awvalid(vip_axil_awvalid), .m_axi_awready(vip_axil_awready),
        .m_axi_wdata(s_axil_wdata), .m_axi_wstrb(s_axil_wstrb),
        .m_axi_wvalid(vip_axil_wvalid), .m_axi_wready(vip_axil_wready),
        .m_axi_bresp(s_axil_bresp), .m_axi_bvalid(vip_axil_bvalid),
        .m_axi_bready(vip_axil_bready), .m_axi_araddr(s_axil_araddr),
        .m_axi_arprot(s_axil_arprot), .m_axi_arvalid(s_axil_arvalid),
        .m_axi_arready(s_axil_arready), .m_axi_rdata(s_axil_rdata),
        .m_axi_rresp(s_axil_rresp), .m_axi_rvalid(vip_axil_rvalid),
        .m_axi_rready(vip_axil_rready)
    );
    ...
    // AW/W ordering is generated by the AMD VIP transaction itself.  Keep
    // these request channels transparent so the VIP and DUT observe the same
    // handshakes.
    assign s_axil_awvalid = vip_axil_awvalid;
    assign vip_axil_awready = s_axil_awready;
    assign s_axil_wvalid = vip_axil_wvalid;
    assign vip_axil_wready = s_axil_wready;
    assign s_axil_bready = test_ctrl_if.hold_axil_b_channel
        ? 1'b0 : vip_axil_bready;
    assign vip_axil_bvalid = test_ctrl_if.hold_axil_b_channel
        ? 1'b0 : s_axil_bvalid;
    assign s_axil_rready = test_ctrl_if.hold_axil_r_channel
        ? 1'b0 : vip_axil_rready;
    assign vip_axil_rvalid = test_ctrl_if.hold_axil_r_channel
        ? 1'b0 : s_axil_rvalid;

    assign test_ctrl_if.axil_awvalid_observed = s_axil_awvalid;
    assign test_ctrl_if.axil_awready_observed = s_axil_awready;
    assign test_ctrl_if.axil_wvalid_observed = s_axil_wvalid;
    assign test_ctrl_if.axil_wready_observed = s_axil_wready;
    assign test_ctrl_if.axil_bvalid_observed = s_axil_bvalid;
    assign test_ctrl_if.axil_bready_observed = s_axil_bready;
    assign test_ctrl_if.axil_bresp_observed = s_axil_bresp;
    assign test_ctrl_if.axil_arvalid_observed = s_axil_arvalid;
    assign test_ctrl_if.axil_arready_observed = s_axil_arready;
    assign test_ctrl_if.axil_rvalid_observed = s_axil_rvalid;
    assign test_ctrl_if.axil_rready_observed = s_axil_rready;
    assign test_ctrl_if.axil_rresp_observed = s_axil_rresp;
