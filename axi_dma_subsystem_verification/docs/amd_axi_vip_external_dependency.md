...
Run an elaboration-only check with:
...
The first functional smoke test starts all five VIP agents, reads both
AXI-Lite channel windows, then performs independent write/readback operations
through `ext_m0 -> mem0` and `ext_m1 -> mem1`:
