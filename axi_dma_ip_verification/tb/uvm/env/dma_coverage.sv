...
class dma_coverage extends uvm_subscriber #(axil_item);
    ...
    covergroup descriptor_cg with function sample(
        ...
        cp_ranges_overlap: coverpoint ranges_overlap {
            bins separate = {1'b0};
            bins overlap  = {1'b1};
        }
    ...
    function automatic bit descriptor_ranges_overlap(
        bit [15:0] src_addr,
        bit [15:0] dst_addr,
        bit [19:0] byte_length
    );
        ...
        src_end = {5'd0, src_addr} + byte_length;
        dst_end = {5'd0, dst_addr} + byte_length;
        return ({5'd0, src_addr} < dst_end) &&
               ({5'd0, dst_addr} < src_end);
    endfunction
