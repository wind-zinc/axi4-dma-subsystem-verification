    ...
    task drive_write(axil_item tr);
        ...
        fork
            ...
            begin : receive_b
                ...
                /* In the directed SVA test, wait until the response really
                 * exists and then leave it stalled for the requested number
                 * of cycles.  Normal transactions keep the original zero-
                 * delay behavior. */
                if (tr.bready_delay != 0) begin
                    do begin
                        @(vif.drv_cb);
                        b_wait++;
                        if (b_wait >= MAX_WAIT_CYCLES)
                            `uvm_fatal("AXIL_B_TIMEOUT",
                                "BVALID timeout before delayed BREADY")
                    end while (!vif.drv_cb.bvalid);

                    repeat (tr.bready_delay)
                        @(vif.drv_cb);
                end
    ...
    task drive_read(axil_item tr);
        ...
        fork
            ...
            begin : receive_r
                ...
                if (tr.rready_delay != 0) begin
                    do begin
                        @(vif.drv_cb);
                        r_wait++;
                        if (r_wait >= MAX_WAIT_CYCLES)
                            `uvm_fatal("AXIL_R_TIMEOUT",
                                "RVALID timeout before delayed RREADY")
                    end while (!vif.drv_cb.rvalid);

                    repeat (tr.rready_delay)
                        @(vif.drv_cb);
                end
